...
export function EditAgentModal({ agent, onClose }: EditAgentModalProps) {
  ...
  const save = () => {
    ...
    const trimmedDescription = description.trim() || '新規ハーネス';
  ...
  return (
    ...
    >
      ...
      <div onClick={(e) => e.stopPropagation()} style={{ width: 940, maxWidth: '95vw' }}>
        <PixelPanel variant="dialog" title="エージェント編集" style={{ padding: 16 }} noPadding>
          <div style={{
            ...
            <Section label="エンジン" hint="プロバイダー · モデル · 次回の再起動時">
              <Row label="プロバイダー">
                <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
              ...
              <span style={{ fontSize: 12, color: 'var(--cth-ink-500)', lineHeight: '16px' }}>
                エンジンの変更は次回の再起動時に反映されます。稼働中のセッションを今すぐ新しいプロバイダー/モデルで再起動するには、コマンドセンター → フロアを使用してください。
              </span>
            </Section>

              </div>
              <div style={{ minWidth: 0 }}>
            <Section label="ブリーフィング" hint="説明 · ゴール">
              <Row label="説明">
                <input
                  value={description}
                  onChange={(e) => setDescription(e.target.value)}
                  placeholder="このエージェントの目的"
                  style={inputStyle}
                />
              </Row>
