...
const COLUMNS: { key: Status; label: string; accent: string }[] = [
  ...
  { key: 'doing',   label: '作業中',   accent: 'var(--cth-lemon)' },
  { key: 'blocked', label: 'ブロック中', accent: 'var(--cth-coral)' },
  { key: 'done',    label: '完了',     accent: 'var(--cth-mint)' }
];
...
export function parseTasks(raw: unknown): HiveTask[] {
  ...
  return list
    ...
    .map((t, i) => ({
      ...
      title: typeof t.title === 'string' ? t.title : '（無題）',
    ...
    <div style={{ flex: 1, minHeight: 0, display: 'flex', flexDirection: 'column', background: 'var(--cth-paper-200)', position: 'relative' }}>
      ...
      <div style={{
        ...
        <span style={{ marginLeft: 'auto', fontSize: 11, color: 'var(--cth-ink-300)' }}>
          新しい仕事？ Michael（モニタータブ）にディスパッチして
        </span>
      </div>
  ...
  return (
    <div style={{ position: 'relative', display: 'flex' }}>
      ...
      >
        ...
        {waitsOnHuman(task) && (
          <span title="あなたの回答待ち — ASK MEタブを確認" style={{
        ...
        <PixelPanel variant="dialog" title="タスク" noPadding style={{ display: 'flex', flexDirection: 'column', width: '100%', minHeight: 0 }}>
          <div style={{ padding: 14, display: 'flex', flexDirection: 'column', gap: 10, minHeight: 0, overflowY: 'auto' }}>
            ...
            <div style={{
              ...
              {task.description?.trim() || <span style={{ color: 'var(--cth-ink-300)' }}>（このカードには説明がありません）</span>}
            </div>
...
function PriorityDots({ level }: { level: number }) {
  ...
  return (
    <span title={`優先度 ${level}/5`} style={{ display: 'inline-flex', gap: 1, flexShrink: 0, marginTop: 2 }}>
      {[1, 2, 3, 4, 5].map((i) => (
        <span key={i} style={{
          width: 4, height: 8,
