...
export function CommandBar({ accent, busy, blocked, onSend }: CommandBarProps) {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
      ...
      <PixelPanel variant="inset" noPadding style={{ padding: 6 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
          ...
          <input
            ...
            onKeyDown={onKey}
            placeholder={blocked ? "Adaがあなたの入力を待っています…" : busy ? "Adaが作業中…" : "コマンドを入力"}
            style={{
              flex: 1,
              padding: '4px 6px 2px',
      ...
      {busy && <span style={{ fontSize: 12, color: 'var(--cth-ink-500)' }}>Adaがツール実行中です。キューに入れるか一時停止してください。</span>}
