    ...
    <div style={{ flex: 1, minHeight: 0, overflowY: 'auto', background: 'var(--cth-paper-200)', padding: 10, display: 'flex', flexDirection: 'column', gap: 10, fontFamily: 'var(--cth-font-mono)' }}>
      {waiting.length === 0 && (
        <div style={{ textAlign: 'center', padding: '24px 12px', color: 'var(--cth-ink-500)', fontSize: 12 }}>
          今あなたの対応が必要なものはありません。 🌿<br />
          <span style={{ fontSize: 11, color: 'var(--cth-ink-300)' }}>
            チームがタスクをあなたの入力でブロックしたとき — 回答すべき質問や、あなただけが実行できる作業 —
            ここ（およびフロアの ASK ME ボード）に表示されます。
          </span>
        </div>
      )}
      {waiting.map((t) => {
          ...
          <div key={t.id} style={{
            ...
            <div style={{
              ...
              <button
                ...
                title="却下 — 回答せずにこの質問を ASK ME ボードから外します (履歴は保持されます)"
                aria-label="この質問を却下"
                style={{
            ...
            <div style={{ padding: 9, display: 'flex', flexDirection: 'column', gap: 8 }}>
              ...
              <textarea
                ...
                rows={3}
                placeholder="回答を入力 — または結果とともに「完了」と送信… (Ctrl+Enterで送信)"
                style={{
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                ...
                >
                  {sending === t.id ? '送信中…' : '回答してブロック解除'}
                </PixelButton>
                {(t.humanQA?.filter((e) => e.a).length ?? 0) > 0 && (
                  <button
                    onClick={() => openTaskDetail(t.id)}
                    title="タスク詳細とQ&A履歴全体を開く"
                    style={{
                  ...
                  >
                    過去の回答 {t.humanQA!.filter((e) => e.a).length} 件を見る
                  </button>
                )}
              </div>
              ...
              {stuck.length > 0 && (
                <div style={{ display: 'flex', flexDirection: 'column', gap: 3 }}>
                  ...
                  {stuck.length > 6 && (
                    <div style={{ paddingLeft: 14, fontSize: 11, color: 'var(--cth-ink-300)' }}>… 他 {stuck.length - 6} 件</div>
                  )}
                </div>
              )}
            </div>
