...
<div class="stage">
  ...
  <section class="page p1">
    <div class="content center">
      ...
      <p class="lede" style="font-size:clamp(1.05rem,2.1vw,1.3rem);margin-top:.5em">
        Windows がついにオフィスに仲間入りしたリリース — そして初回実行が静かに失敗する問題も直りました。
      </p>
      <div class="stat">
        <div><b>27</b><span>件の修正</span></div>
        <div><b>4</b><span>つの新画面</span></div>
        <div><b>1</b><span>つのプラットフォーム復活</span></div>
      </div>
    </div>
    <div class="nav">
      <div class="dots">
      ...
      <label class="btn primary" for="pg2">開始 &rarr;</label>
    </div>
  </section>

  <section class="page p2">
    <div class="content">
      <p class="kicker">ハイライト</p>
      <h1>Windows でもエージェント同士が会話できるように。</h1>
      <p class="lede">ダウンロードの約半数は Windows から。そこではエージェント間メッセージングが
      これまで一度も動いていませんでした。</p>
      <div class="placeholder" data-label="メッセージを交わす2体のエージェント" style="aspect-ratio:24/9"></div>
      <p class="quote">どのエージェントも起動し、描画され、完全に健康そうに見えました。
      しかし受信箱があることを知らされたエージェントは1体もいなかったのです。</p>
      <p style="margin-top:16px;color:var(--ink-soft);font-size:14px">.exe 以外のCLIは
      cmd.exe 経由で起動されており、複数行の引数が最初の改行で切断され、
      プロトコルブロックも一緒に失われていました。現在は実際のインタプリタを引数配列で
      起動するため、プロンプト全体が届きます。</p>
    </div>
    <div class="nav">
      <div class="dots">
  ...
  <section class="page p3">
    <div class="content">
      ...
      <h1>セットアップが終わったら、オフィスが目覚めます。</h1>
      <p class="lede">新規ユーザーが最初に行う操作に、4つの別々のバグが潜んでいました。</p>
      <ul class="rows">
        <li><i>ウィザード</i><div><b>候補フォルダが正しく機能するように</b>
          <p>~/HarnessAgents を受け入れるとチルダが文字通り保存され ENOENT で落ちていました。
          現在は実際のパスに解決され、フィールドもその候補を表示します。</p></div></li>
        <li><i>ウィザード</i><div><b>ステップ1の時点で通知</b>
          <p>以前は空フォルダでも4ステップすべてを進めてから弾かれていました。短い画面での
          パネルあふれも解消されました。</p></div></li>
        <li><i>Hive</i><div><b>サービスはセットアップ時に起動（次回起動時ではない）</b>
          <p>新規インストールでは再起動するまでメッセージルータ・フック・テレメトリが停止したまま
          になり、メールは届かずエージェントも報告しませんでした。</p></div></li>
        <li><i>エージェント</i><div><b>「再起動して続行」に再開先があるように</b>
          <p>ライブセッションIDを第二ソースからも記録するため、フックが届かなかった場合でも
          続行できます。</p></div></li>
      </ul>
    </div>
    <div class="nav">
    ...
    <div class="content">
      ...
      <div class="split">
        <div class="card">
          ...
          <h2>エージェントが使える全スキル</h2>
          <p>Claude Code・OpenCode・Codex にインストール済みのスキル一覧と、検索・フィルタ・
          インストール/アンインストール対応の227件以上のカタログ。</p>
        </div>
        <div class="card">
          <span class="tag">前提ツール</span>
          <h2>必要なツールが揃っているか一目で判別</h2>
          <p>MemPalace、uv、git、各エージェントエンジンの稼働状況とディスク上の場所を表示。
          ボタンひとつで Michael に不足分を依頼できます。</p>
        </div>
        <div class="card">
          <span class="tag">リリースドロップ</span>
          <h2>このページ</h2>
          <p>アップデートノートは以前、隅に3つの途切れた箇条書きでした。リリースが独自の
          デザインページを持てるようになり、あなたは今その第一号を読んでいます。</p>
        </div>
        <div class="card">
          <span class="tag">ダークモード</span>
          <h2>読みやすさのための再構築</h2>
          <p>すべてのコントロール境界が背景比2:1未満で輪郭が見えなくなっていました。
          目分量ではなく測定して調整し直しました。</p>
        </div>
      </div>
    <div class="nav">
      <div class="dots">
  ...
  <section class="page p5">
    <div class="content">
      <p class="kicker">その他</p>
      <h1>残りのリスト。</h1>
      <ul class="rows">
        <li><i>ターミナル</i><div><b>コピーがクリーンに返されるように</b>
          <p>引用バーは取り除かれ、ターミナルはUTF-8で動作するため、emダッシュも
          他アプリへそのまま運ばれます。</p></div></li>
        <li><i>ターミナル</i><div><b>音声入力が話した内容をそのまま貼り付け</b></div></li>
        <li><i>IDE</i><div><b>画像が画像として開くように</b>
          <p>PNG・SVG・埋め込みスクリーンショットを表示。タイトルにはエージェント名が付きます。</p></div></li>
        <li><i>エージェント</i><div><b>「再起動して続行」で停止したエージェントを復旧</b></div></li>
        <li><i>エージェント</i><div><b>モデルピッカーに Grok 4.6 を追加</b></div></li>
        <li><i>エージェント</i><div><b>OpenCode が実際に使えるモデルを実行するように</b></div></li>
        <li><i>ボード</i><div><b>タスクカードが消失しなくなりました</b></div></li>
        <li><i>Hive</i><div><b>奇妙なメッセージIDでも起床ナッジが確実に届くように</b></div></li>
        <li><i>Hive</i><div><b>コンパクト化が毎時ではなく1回だけ発火するように</b></div></li>
        <li><i>Hive</i><div><b>コスト台帳がgit履歴から外れるように</b></div></li>
        <li><i>オフィス</i><div><b>誰も見ていないときオフィスの描画を停止</b></div></li>
        <li><i>レイアウト</i><div><b>Michael がドックの先頭に戻りました</b></div></li>
      </ul>
    </div>
    <div class="nav">
      <div class="dots">
  ...
  <section class="page p6">
    <div class="content center">
      <p class="kicker">最後にひとつ</p>
      <h1 style="font-size:clamp(1.9rem,4.6vw,2.9rem)">自分のマシンで動かしてくれて
      ありがとうございます。</h1>
      <p class="lede" style="margin-top:.4em">ここにあるすべてのエージェントは、あなたの
      ハードウェア上、あなたのフォルダ内、あなたのキーで動きます。その点は何も変わりません。</p>
      <p class="quote">役に立っていると感じたら、スターこそがマーケティング予算の全額です。
      ボタンはこのページのすぐ下にあります。</p>
    </div>
    <div class="nav">
      <div class="dots">
        <label class="dot" for="pg1"></label><label class="dot" for="pg2"></label>
      ...
      <label class="btn" for="pg1">最初へ</label>
    </div>
  </section>
