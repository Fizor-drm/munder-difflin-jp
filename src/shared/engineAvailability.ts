...
export function engineAvailabilityBadge(a: EngineAvailability): string | null {
  switch (a.state) {
    ...
    case 'installs-on-first-run': return '初回起動時にインストール';
...
export function engineAvailabilityMessage(a: EngineAvailability, label: string): string | null {
  ...
  return `${label} がこのコンピュータにインストールされておらず、アプリにはインストーラもないため、` +
    `Michael を開始できませんでした。先にインストールしてから「再確認」を押してください。` +
    `または、初回起動時に自動インストールされる Claude Code を選んでください。`;
}
